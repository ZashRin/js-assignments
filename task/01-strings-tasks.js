...
function getRectangleString(width, height) {
    let upLeft = '┌', upRight = '┐', botLeft = '└', botRight = '┘',
    ubBorder = '─', sideBorder = '│';
    let rect = '';
    for(let i = 0; i < height - 1; i++){
        if(i == 0) rect = upLeft + ubBorder.repeat(width - 2) + upRight + '\n';
        if(i == height - 2) rect += botLeft + ubBorder.repeat(width - 2) + botRight + '\n';
        else rect += sideBorder + ' '.repeat(width - 2) + sideBorder + '\n';
    }
    return rect;
...
function encodeToRot13(str) {
    let t = str.split(' ');
    t.forEach((item, i, arr) => arr[i] = item.split(''));
    t.forEach((item, i, arr) => {
        item.forEach((item, i, arr) =>{
            let code = item.charCodeAt(0);
            (code >= 65 && code < 78) || (code >= 97 && code < 110) ? item = String.fromCharCode(code + 13) :
            (code >= 78 && code < 91) || (code >= 110 && code < 123) ? item = String.fromCharCode(code - 13) :
            item = item;
            arr[i] = item;
        });
        arr[i] = item.join('');
